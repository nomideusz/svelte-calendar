...
<script lang="ts">
	...
	// ─── Buffer config ─────────────────────────────
	const INITIAL_BUFFER = 8;    // ±8 weeks on load; scroll-extend covers the rest
	const EXTEND_BY = 8;         // +8 weeks when the user hits an edge (the store merges, so only the new slice is news)
	...
	const eventHours = $derived.by((): [number, number] => {
		let lo = 24;
		let hi = -1;
	...
	function timeTop(msIntoDay: number): number {
	...
	function chipTops(list: TimelineEvent[], dayMs: number): number[] {
		const tops: number[] = [];
		let min = 0;
	...
	function nowTop(list: TimelineEvent[], tops: number[], dayMs: number): number {
	...
	const weeks = $derived.by(() => {
		...
		for (let w = -bufferBefore; w <= bufferAfter; w++) {
			...
			for (let d = 0; d < customDays; d++) {
				...
				days.push({ ms, dayNum, isToday, isPast, isWeekend, monthLabel: null, events: timedEvents, allDaySegments });
			}

			result.push({ weekStart: periodStart, isCurrent, monthLabel: '', monthStart: false, days });
		}
	...
	function watchTouch(root: HTMLElement) {
		const on = () => { touched = true; };
	...
	const periodOf = (ms: number) => (customDays === 7 ? sowFn(ms, mondayStart) : sod(ms));
	$effect(() => {
	...
	function handleUserScroll() {
		...
		if (el.scrollTop < EDGE_PX) {
			...
			const first = el.querySelector<HTMLElement>('[data-week]');
			const key = first?.dataset.week;
			const offset = first ? first.offsetTop - el.scrollTop : 0;
			bufferBefore += EXTEND_BY;
			tick().then(() => {
				const row = key ? el.querySelector<HTMLElement>(`[data-week="${key}"]`) : null;
	...
	function dayDropStart(ms: number): Date | null {
	...
	function handleDayCellClick(ms: number, e: Event) {
		...
		oneventcreate({ start, end: new Date(start.getTime() + durMin * 60_000) });
	}
	...
	let dropDayMs = $state<number | null>(null);

	function onCellDragOver(e: DragEvent, ms: number) {
	...
	function onCellDragLeave(ms: number) {
		if (dropDayMs === ms) dropDayMs = null;
	}

	function onCellDrop(e: DragEvent, ms: number) {
		dropDayMs = null;
		if (!onexternaldrop || readOnly || !e.dataTransfer) return;
		const start = dayDropStart(ms);
		if (!start) return;
		e.preventDefault();
		onexternaldrop({ start, dataTransfer: e.dataTransfer });
	}
	...
	const fit = createChipFit({
		slot: '.wg-cell',
		fonts: { title: '.wg-probe .wg-ev-title', time: '.wg-probe .wg-ev-time', room: '.wg-probe .wg-ev-loc' },
	});
	/** A cell that carries a month label is this much narrower (.wg-cell--month). */
	const MONTH_INSET = 22;
	function chipParts(ev: TimelineEvent, inset = 0): Record<string, boolean> {
		return fit.parts(
			[
	...
	function weekMsAtY(clientY: number): number {
		const rows = el?.querySelectorAll<HTMLElement>('[data-week]');
	...
	function updateDragFromPointer() {
		...
		const weekOffset =
			evStartWeekMs && nowWeekMs
				? Math.round((nowWeekMs - evStartWeekMs) / (customDays * DAY_MS))
				: 0;
		const deltaDays = dayOffset + weekOffset * customDays;
		drag.updatePointer(
			new Date(addDaysMs(ev.start.getTime(), deltaDays)),
			new Date(addDaysMs(ev.end.getTime(), deltaDays)),
		);
	}
	...
	const AUTO_EDGE = 56;   // px from an edge where the pull starts
	const AUTO_STEP = 14;   // px per frame
	let autoDir = 0;
	let autoRaf = 0;
	...
	function onCellKeydown(e: KeyboardEvent, ms: number) {
		...
		for (let i = 0; i < 7; i++) {
			const cell = el?.querySelector<HTMLElement>(`[data-day="${target}"]`);
			if (cell) {

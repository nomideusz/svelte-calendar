...
import type { HeadlessCalendar, HeadlessCalendarOptions } from './types.js';
import type { TimelineEvent } from '../core/types.js';

// Monday 2025-03-03 — fixed so ISO weekday assertions are stable
const DAY = (h: number, m = 0) => new Date(2025, 2, 3, h, m);

function makeEvent(id: string, startH: number, endH: number): TimelineEvent {
	return {
		id,
		title: `Event ${id}`,
		start: DAY(startH),
...
async function withCalendar(
	options: Partial<HeadlessCalendarOptions> & { adapter?: HeadlessCalendarOptions['adapter'] },
	fn: (cal: HeadlessCalendar) => Promise<void> | void,
): Promise<void> {
	let cal!: HeadlessCalendar;
	const destroy = $effect.root(() => {
		cal = createCalendar({
			adapter: options.adapter ?? createMemoryAdapter(),
			initialDate: DAY(0),
			...options,
		} as HeadlessCalendarOptions);
	});
	try {
		flushSync();
...
describe('drag-to-create lifecycle', () => {
	...
	it('rejects a create overlapping a blocked slot', async () => {
		const oncreate = vi.fn();
		await withCalendar(
			{ oneventcreate: oncreate, blockedSlots: [{ day: 1, start: 12, end: 14 }] },
			async (cal) => {
	...
	it('rejects a create on a disabled date', async () => {
		const oncreate = vi.fn();
		await withCalendar(
			{ oneventcreate: oncreate, disabledDates: [DAY(0)] },
			async (cal) => {
...
describe('resize lifecycle', () => {
	...
	it('rejects a resize landing on a blocked slot and leaves the event untouched', async () => {
		...
		const adapter = createMemoryAdapter([makeEvent('1', 9, 10)]);
		await withCalendar(
			{ adapter, oneventmove: onmove, blockedSlots: [{ day: 1, start: 12, end: 14 }] },
			async (cal) => {
				cal.dragState.beginResize('1', 'end', DAY(9), DAY(10));

...
const at = (day: number, h: number, m = 0) => new Date(2025, 2, 3 + day, h, m);

function ev(id: string, day: number, startH: number, endH: number): TimelineEvent {
	return { id, title: `Event ${id}`, start: at(day, startH), end: at(day, endH) };
}

async function withAgenda(
	options: Partial<RangeAgendaOptions>,
	fn: (agenda: HeadlessRangeAgenda) => Promise<void> | void,
): Promise<void> {
	let agenda!: HeadlessRangeAgenda;
	const destroy = $effect.root(() => {
		agenda = createRangeAgenda({
			adapter: options.adapter ?? createMemoryAdapter(),
			initialDate: MON,
			...options,
		} as RangeAgendaOptions);
	});
	try {
		flushSync();

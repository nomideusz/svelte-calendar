...
export function fits(text: string, font: string, width: number, lines = 1): boolean {
	return lineCount(text, font, width) <= lines;
}

/** Height the text will take at `width`, before it is in the DOM. */
export function textHeight(text: string, font: string, width: number, lineHeight: number): number {
...
export function pickFit(candidates: readonly string[], font: string, width: number, lines = 1): string {
	return candidates.find((c) => fits(c, font, width, lines)) ?? candidates.at(-1) ?? '';
}

// ─── Chip parts ─────────────────────────────────────────────────────────
// A dense chip holds more than fits: a title, a time, a room. CSS can only
// shrink-and-ellipsise, which cuts a title mid-letter to make room for a
// detail nobody asked for. This decides instead: measure what each part
// costs, keep the anchors, and add the rest while they fit.
...
export function fitParts(
	parts: readonly ChipPart[],
	budget: number,
	measure: (text: string, font: string) => number = textWidth,
	...
	const cost = (p: ChipPart) => (p.size ?? measure(p.text!, p.font ?? '')) + (p.extra ?? 0);
	let used = 0;
 ...
 */
export function fitLabel(candidates: readonly string[], lines = 1): Attachment<HTMLElement> {
	return (el) => {
...
export function breakLines(text: string, font: string, width: number): string[] {
	...
	words.forEach((w, wi) => {
		...
		frags.forEach((f, fi) => {
			boxes.push(f);
			if (fi < frags.length - 1) glue.push('shy');
		});
		if (wi < words.length - 1) glue.push('space');
	});
	...
	const badness = (slackPx: number, spaces: number) => {
	...
	const best = [new Array<number>(n + 1).fill(Infinity), new Array<number>(n + 1).fill(Infinity)];
	const prev = [new Array<[number, number]>(n + 1), new Array<[number, number]>(n + 1)];
	best[0][0] = 0;
	for (let i = 0; i < n; i++) {
		for (const ph of [0, 1]) {
			if (best[ph][i] === Infinity) continue;
			let w = 0;
			let spaces = 0;
			for (let j = i; j < n; j++) {
				w += bw[j];
	...
	if (best[h][n] === Infinity) return [text]; // a box wider than the column: let the browser cope

	const lines: string[] = [];
	for (let k = n; k > 0; ) {
		const [i, ph] = prev[h][k];
		let line = '';
		for (let b = i; b < k; b++) line += boxes[b] + (b < k - 1 && glue[b] === 'space' ? ' ' : '');
		if (h) line += '-';
		lines.unshift(line);
		k = i;
		h = ph;
 ...
 */
export function typeset(text: string): Attachment<HTMLElement> {
	return (el) => {
		if (getComputedStyle(el).whiteSpace !== 'normal') return;
		const font = fontOf(el);
		let width = 0;
		...
		const set = (w: number) => {
			...
			for (const s of el.children) if (s.scrollWidth > s.clientWidth + 1) return false;
			return true;
		};
		const apply = () => {
			if (!width) return;
			for (const w of [width, width - 2, width * 0.985, width * 0.97]) if (set(w)) return;
			el.textContent = text;
		};
		const ro = new ResizeObserver(([e]) => {
			if (e.contentRect.width === width) return;

<!--
  Planner day mode — Hey-inspired horizontal filmstrip.
...
<script lang="ts">
	...
	/** Collapsed-hours blocks: track which days have expanded hidden hours */
	let nightExpanded = $state<Set<number>>(new Set());

	// ─── Night block constants ──────────────────────────────
	/** Night = 22:00 → 06:00 (8 hours). Collapsed width = small strip. */
	const visibleStartHour = $derived(visibleHours?.[0] ?? 6);
	const visibleEndHour = $derived(visibleHours?.[1] ?? 22);
	const visibleHoursCount = $derived(Math.max(1, visibleEndHour - visibleStartHour));
	const collapsedHoursCount = $derived(24 - visibleHoursCount);
	...
	// Each day has: visible hours at full width + hidden-hours block between days.

	const dayW_day = $derived(visibleHoursCount * hourWidth);
	const nightW_expanded = $derived(collapsedHoursCount * hourWidth);
	...
	function timeToPx(ms: number): number {
		for (let i = 0; i < dayLayouts.length; i++) {
			...
			if (hourOfDay < visibleStartHour) {
				...
				if (i > 0) {
					...
					if (prev.nightOpen) {
						...
						const nightFrac = (hourOfDay + (24 - visibleEndHour)) / collapsedHoursCount;
			...
			if (hourOfDay >= visibleEndHour) {
				// 22:00+ — in this day's night block
				if (d.nightOpen) {
					const nightFrac = (hourOfDay - visibleEndHour) / collapsedHoursCount;
			...
			// Daytime (06:00–22:00)
			const dayFrac = (hourOfDay - visibleStartHour) / visibleHoursCount;
	...
	function pxToTime(px: number): number {
		...
		if (dayLayouts.length > 0 && px < dayLayouts[0].dayX) {
			...
			return d.ms + visibleStartHour * HOUR_MS - (before / hourWidth) * HOUR_MS;
		}
		...
		for (let i = 0; i < dayLayouts.length; i++) {
			...
			if (px >= d.dayX && px < d.dayX + d.dayWidth) {
				...
				const hourOfDay = visibleStartHour + frac * visibleHoursCount;
			...
			if (px >= d.nightX && px < d.nightX + d.nightW) {
				...
				const hoursIntoNight = frac * collapsedHoursCount;
				return d.ms + (visibleEndHour + hoursIntoNight) * HOUR_MS;
			}
		}
		...
		if (last) {
			...
			return last.ms + DAY_MS + visibleStartHour * HOUR_MS + (past / hourWidth) * HOUR_MS;
		}
		return origin;
	}
	...
	function handleTrackClick(e: MouseEvent) {
		...
		for (const d of dayLayouts) {
			if (clickX >= d.dayX && clickX < d.dayX + d.dayWidth) {
				...
				const hour = Math.floor(visibleStartHour + frac * visibleHoursCount);
...
<div class="fs" style={style || undefined} style:height="{height}px" role="region" aria-label="Day grid">
	<div
		class="fs-scroll"
		...
		role="application"
		aria-label="Scrollable day grid — drag to scroll"
	>
		<div class="fs-track" style:width="{totalW()}px" onclick={handleTrackClick} role="none">
			...
			{#each dayLayouts as d, i (d.ms)}
				...
				>
					...
					{#each { length: visibleHoursCount } as _, h}
						{@const hour = visibleStartHour + h}
						{@const x = h * hourWidth}
				...
				{#if i < count - 1}
					...
					>
						{#if d.nightOpen}
							...
							{#each { length: collapsedHoursCount } as _, h}
								{@const hour = (visibleEndHour + h) % 24}
								{@const x = h * hourWidth}
